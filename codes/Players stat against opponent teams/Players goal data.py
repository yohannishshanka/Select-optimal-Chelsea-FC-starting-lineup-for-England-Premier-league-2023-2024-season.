...
excel_file_path = 'players goal data.xlsx'  # Use an Excel file for multiple sheets
...
file_path = 'players goal data.xlsx'
...
new_file_path = 'players goal data.xlsx'
...
# Load the workbook
workbook = load_workbook('players goal data.xlsx')
...
workbook.save('players goal data.xlsx')
...
for url in urls:
    ...
    for new_url, player_name in zip(new_urls, first_parts_modified_new):
       ...
       # Extract table rows
        ...
        for row in rows[1:]:
            ...
            row_data = [cells[1].get_text(strip=True), cells[6].get_text(strip=True)]
            ...
            workbook = openpyxl.load_workbook('players goal data.xlsx')
        ...
        for row in rows[1:]:
           ...
           # Iterate over each sheet in the workbook
            for sheet in workbook.sheetnames:
             ...
             # Save the workbook
            workbook.save('forward player goal data.xlsx')
    ...
    for url, data in filtered_table_data_dict.items():
        midfield_indices = [index for index, row in enumerate(data) if row[0] ==  "Left-Back"]
        #print("Goalkeeper Indices:", goalkeeper_indices)
    ...
    if items_table:
        ...
        for index in midfield_indices:
            a.append(midfield_indices[index])
    ...
    midfield_href_list = []
      ...
      # Iterate through each index in goalkeeper_indices
    for index in midfield_indices:
        midfield_href_list.append(filtered_href_list[index])
        
    # Append the corresponding href link from filtered_href_list to goalkeeper_href_list
    #print(midfield_href_list)
    ...
    # Iterate through each filtered href link
    for href in midfield_href_list:
        # Split the href link by "/"
        parts = href.split("/")
    ...
    for url, data in filtered_table_data_dict.items():
        midfield_indices = [index for index, row in enumerate(data) if row[0] ==  "Centre-Back"]
        #print("Goalkeeper Indices:", goalkeeper_indices)
    ...
    for url, data in filtered_table_data_dict.items():
        midfield_indices = [index for index, row in enumerate(data) if row[0] == "Right-Back"]
        #print("Goalkeeper Indices:", goalkeeper_indices)
