...
excel_file_path = 'Goalkeeper conceeded goal data.xlsx'  # Use an Excel file for multiple sheets
...
file_path = 'Goalkeeper conceeded goal data.xlsx'
...
new_file_path = 'Goalkeeper conceeded goal data.xlsx'
...
# Load the workbook
workbook = load_workbook('Goalkeeper conceeded goal data.xlsx')
...
workbook.save('Goalkeeper conceeded goal data.xlsx')
...
for url in urls:
    ...
    for url, data in filtered_table_data_dict.items():
        goalkeeper_indices = [index for index, row in enumerate(data) if row[0] ==  "Goalkeeper"]
    ...
    # Iterate through each filtered href link
    for href in goalkeeper_href_list:
    ...
    for new_url, player_name in zip(new_urls, first_parts_modified_new):
       ...
       # Extract table rows
        ...
        for row in rows[1:]:
            ...
            workbook = openpyxl.load_workbook('Goalkeeper conceeded goal data.xlsx')
