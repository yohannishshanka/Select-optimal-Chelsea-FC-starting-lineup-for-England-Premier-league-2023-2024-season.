...
#filter players can play second striker
filtered_df = df[((df['Position'] == 'Centre-Forward') |df['Other Positions'].str.contains('Second Striker') |(df['Position'] == 'Second Striker') |df['Other Positions'].str.contains('Centre-Forward'))  ]
...
def select_best_second_striker(Market_value,apperence,position_list,seasonal_apperence, goals, assists, againts_team_goals, againts_team_assists, Shots, ShotsOnTarget, Penalty, Free_kick, Big_chances_missed, Shooting_accuracy, Big_chances_created, Crosees, Passes, Off_side,  Tackles_per_game, Block_shots_per_game, interceptions_per_game, Total_clearance_per_game,seasonal_goals,seasonal_assists,seasonal_Yellow,seasonal_Red,seasonal_Second_Yellow,recent_Goals,recent_Assists,recent_Shots,recent_ShotOnTarget,recent_Fouls,recent_Yellow_card,recent_Red_card,recent_Offside,recent_Pass_accuracy,recent_Keypasses, weights):
    ...
    # Define variables
    num_second_striker = len(goals)
    x = [solver.IntVar(0, 1, f'x{i}') for i in range(num_second_striker)]

    # Define objective function
    objective = solver.Objective()
    for i in range(num_second_striker):
        objective.SetCoefficient(x[i],weights['Market_value'] * Market_value[i]+weights['position_list'] * position_list[i]+ weights['goals'] * (goals[i] / apperence[i]) + weights['assists'] * (assists[i] / apperence[i]) + weights['againts_team_goals'] * againts_team_goals[i] + weights['againts_team_assists'] * againts_team_assists[i] + weights['Shots'] * Shots[i] + weights['Penalty'] * Penalty[i] + weights['ShotsOnTarget'] * ShotsOnTarget[i] + weights['Free_kick'] * Free_kick[i] - weights['Big_chances_missed'] * Big_chances_missed[i] + weights['Shooting_accuracy'] * Shooting_accuracy[i] + weights['Big_chances_created'] * Big_chances_created[i] + weights['Crosees'] * Crosees[i] + weights['Passes'] * Passes[i] - weights['Off_side'] * Off_side[i]  +weights['seasonal_goals'] * (seasonal_goals[i] / seasonal_apperence[i])+weights['seasonal_assists'] * (seasonal_assists[i] / seasonal_apperence[i])-weights['seasonal_Yellow'] * seasonal_Yellow[i]-weights['seasonal_Red'] * seasonal_Red[i]- weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[i]+weights['recent_Goals'] * recent_Goals[i]+weights['recent_Assists'] * recent_Assists[i]+weights['recent_Shots'] * recent_Shots[i]+weights['recent_ShotOnTarget'] * recent_ShotOnTarget[i]-weights['recent_Fouls'] * recent_Fouls[i]-weights['recent_Yellow_card'] * recent_Yellow_card[i]-weights['recent_Red_card'] * recent_Red_card[i]-weights['recent_Offside'] * recent_Offside[i]+weights['recent_Pass_accuracy']*recent_Pass_accuracy[i]+weights['recent_Keypasses']*recent_Keypasses[i])
    objective.SetMaximization()
    
    # Add constraint
    solver.Add(sum(x[i] for i in range(num_second_striker)) == 2)  # Select 2 forwards
    
    # Solve the problem
    status = solver.Solve()
    
    if status == pywraplp.Solver.OPTIMAL:
        selected_second_striker = [i for i in range(num_second_striker) if x[i].solution_value() == 1]
        return selected_second_striker
    else:
        return None, None

#data
names = filtered_df["Player"].to_list()
position_list = [1 if position == "Second Striker" else 0 for position in filtered_df["Position"]]
...
weights = {'Market_value':1,'position_list':1,'goals': 4, 'assists': 6,'againts_team_goals' : 4  ,'againts_team_assists' : 6,'Shots': 4+additional_weigth[0],'ShotsOnTarget': 4+additional_weigth[0], 'Penalty': 2+additional_weigth[0],'Free_kick' : 2+additional_weigth[0] ,'Big_chances_missed' : 2,'Shooting_accuracy': 4,'Big_chances_created' : 4 ,'Crosees' : 1,'Passes': 2,'Off_side' : 2,'seasonal_goals':6,'seasonal_assists':8,'seasonal_Yellow':2,'seasonal_Red':2,'seasonal_Second_Yellow':2,'recent_Goals':8,'recent_Assists':10,'recent_Shots':6,'recent_ShotOnTarget':6,'recent_Fouls':4,'recent_Yellow_card':4,'recent_Red_card':4,'recent_Offside':2,'recent_Pass_accuracy':4,'recent_Keypasses':6}

# Select the best and second-best second striker
selected_second_striker = select_best_second_striker(Market_value,apperence,position_list,seasonal_apperence, goals, assists, againts_team_goals, againts_team_assists, Shots, ShotsOnTarget, Penalty, Free_kick, Big_chances_missed, Shooting_accuracy, Big_chances_created, Crosees, Passes, Off_side,  Tackles_per_game, Block_shots_per_game, interceptions_per_game, Total_clearance_per_game,seasonal_goals,seasonal_assists,seasonal_Yellow,seasonal_Red,seasonal_Second_Yellow,recent_Goals,recent_Assists,recent_Shots,recent_ShotOnTarget,recent_Fouls,recent_Yellow_card,recent_Red_card,recent_Offside,recent_Pass_accuracy,recent_Keypasses, weights)
selected_second_striker.sort(key=lambda x:weights['Market_value'] * Market_value[x]+ weights['position_list'] * position_list[x] + weights['goals'] * (goals[x] / apperence[x]) + weights['assists'] * (assists[x] / apperence[x]) + weights['againts_team_goals'] * againts_team_goals[x] + weights['againts_team_assists'] * againts_team_assists[x] + weights['Shots'] * Shots[x] + weights['Penalty'] * Penalty[x] + weights['ShotsOnTarget'] * ShotsOnTarget[x] + weights['Free_kick'] * Free_kick[x] - weights['Big_chances_missed'] * Big_chances_missed[x] + weights['Shooting_accuracy'] * Shooting_accuracy[x] + weights['Big_chances_created'] * Big_chances_created[x] + weights['Crosees'] * Crosees[x] + weights['Passes'] * Passes[x] + weights['Off_side'] * Off_side[x] + weights['seasonal_goals'] * (seasonal_goals[x] / seasonal_apperence[x]) + weights['seasonal_assists'] * (seasonal_assists[x] / seasonal_apperence[x]) - weights['seasonal_Yellow'] * seasonal_Yellow[x] - weights['seasonal_Red'] * seasonal_Red[x] - weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[x] + weights['recent_Goals'] * recent_Goals[x] + weights['recent_Assists'] * recent_Assists[x] + weights['recent_Shots'] * recent_Shots[x] + weights['recent_ShotOnTarget'] * recent_ShotOnTarget[x] - weights['recent_Fouls'] * recent_Fouls[x] - weights['recent_Yellow_card'] * recent_Yellow_card[x] - weights['recent_Red_card'] * recent_Red_card[x] - weights['recent_Offside'] * recent_Offside[x]+weights['recent_Pass_accuracy']*recent_Pass_accuracy[x]+weights['recent_Keypasses']*recent_Keypasses[x], reverse=True)


# Get the names of the two best second striker
best_second_striker_idx = selected_second_striker[0]
second_best_second_striker_idx = selected_second_striker[1]
best_second_striker_name = names[best_second_striker_idx]
second_best_second_striker_name = names[second_best_second_striker_idx]
...
if team_strength=='Big six':
    i=best_second_striker_idx
    per= weights['Market_value'] * Market_value[i]+weights['position_list'] * position_list[i]+ weights['goals'] * (goals[i] / apperence[i]) + weights['assists'] * (assists[i] / apperence[i]) + weights['againts_team_goals'] * againts_team_goals[i] + weights['againts_team_assists'] * againts_team_assists[i] + weights['Shots'] * Shots[i] + weights['Penalty'] * Penalty[i] + weights['ShotsOnTarget'] * ShotsOnTarget[i] + weights['Free_kick'] * Free_kick[i] - weights['Big_chances_missed'] * Big_chances_missed[i] + weights['Shooting_accuracy'] * Shooting_accuracy[i] + weights['Big_chances_created'] * Big_chances_created[i] + weights['Crosees'] * Crosees[i] + weights['Passes'] * Passes[i] - weights['Off_side'] * Off_side[i]  +weights['seasonal_goals'] * (seasonal_goals[i] / seasonal_apperence[i])+weights['seasonal_assists'] * (seasonal_assists[i] / seasonal_apperence[i])-weights['seasonal_Yellow'] * seasonal_Yellow[i]-weights['seasonal_Red'] * seasonal_Red[i]- weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[i]+weights['recent_Goals'] * recent_Goals[i]+weights['recent_Assists'] * recent_Assists[i]+weights['recent_Shots'] * recent_Shots[i]+weights['recent_ShotOnTarget'] * recent_ShotOnTarget[i]-weights['recent_Fouls'] * recent_Fouls[i]-weights['recent_Yellow_card'] * recent_Yellow_card[i]-weights['recent_Red_card'] * recent_Red_card[i]-weights['recent_Offside'] * recent_Offside[i]+weights['recent_Pass_accuracy']*recent_Pass_accuracy[i]+weights['recent_Keypasses']*recent_Keypasses[i]
    sheet.append(('Second Striker', best_second_striker_name,1,per,Jersey_num[i]))
    
elif team_strength=='Good teams':
    if count_2 >6:
        i=second_best_second_striker_idx
        per= weights['Market_value'] * Market_value[i]+weights['position_list'] * position_list[i]+ weights['goals'] * (goals[i] / apperence[i]) + weights['assists'] * (assists[i] / apperence[i]) + weights['againts_team_goals'] * againts_team_goals[i] + weights['againts_team_assists'] * againts_team_assists[i] + weights['Shots'] * Shots[i] + weights['Penalty'] * Penalty[i] + weights['ShotsOnTarget'] * ShotsOnTarget[i] + weights['Free_kick'] * Free_kick[i] - weights['Big_chances_missed'] * Big_chances_missed[i] + weights['Shooting_accuracy'] * Shooting_accuracy[i] + weights['Big_chances_created'] * Big_chances_created[i] + weights['Crosees'] * Crosees[i] + weights['Passes'] * Passes[i] - weights['Off_side'] * Off_side[i]  +weights['seasonal_goals'] * (seasonal_goals[i] / seasonal_apperence[i])+weights['seasonal_assists'] * (seasonal_assists[i] / seasonal_apperence[i])-weights['seasonal_Yellow'] * seasonal_Yellow[i]-weights['seasonal_Red'] * seasonal_Red[i]- weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[i]+weights['recent_Goals'] * recent_Goals[i]+weights['recent_Assists'] * recent_Assists[i]+weights['recent_Shots'] * recent_Shots[i]+weights['recent_ShotOnTarget'] * recent_ShotOnTarget[i]-weights['recent_Fouls'] * recent_Fouls[i]-weights['recent_Yellow_card'] * recent_Yellow_card[i]-weights['recent_Red_card'] * recent_Red_card[i]-weights['recent_Offside'] * recent_Offside[i]+weights['recent_Pass_accuracy']*recent_Pass_accuracy[i]+weights['recent_Keypasses']*recent_Keypasses[i]
        sheet.append(('Second Striker', second_best_second_striker_name,2,per,Jersey_num[i]))
