...
#filter players can play defensive  midfielder
filtered_df = df[((df['Position'] == 'Central Midfield') |df['Other Positions'].str.contains('Central Midfield') |(df['Position'] == 'Defensive Midfield') |df['Other Positions'].str.contains('Defensive Midfield'))  ]
...
additional_weigth=[]
Keeping_possession_weigth=[]
Creating_long_weigth=[]
Stealing_the_ball_weigth=[]
Protecting_the_lead_weigth=[]
Stopping_opponents_weigth=[]


kpi_list = ['Counter attacks', 'Attacking set pieces','Finishing scoring chances', 'Creating chances through individual skill', 'Creating chances using through balls']
...
tasks_strengths['Matched KPIs'] = tasks_strengths['KPIs'].apply(lambda x: 'Keeping possession of the ball' in x)

for index, row in tasks_strengths.iterrows():
    if row['Matched KPIs']:
        Keeping_possession_weigth.append(2)
    else:
        Keeping_possession_weigth.append(0)
...
tasks_strengths['Matched KPIs'] = tasks_strengths['KPIs'].apply(lambda x: 'Protecting the lead' in x)
        
for index, row in tasks_strengths.iterrows():
    if row['Matched KPIs']:
        Protecting_the_lead_weigth.append(2)
    else:
        Protecting_the_lead_weigth.append(0)

tasks_strengths['Matched KPIs'] = tasks_strengths['KPIs'].apply(lambda x: 'Stopping opponents from creating chances' in x)
        
for index, row in tasks_strengths.iterrows():
    if row['Matched KPIs']:
        Stopping_opponents_weigth.append(2)
    else:
        Stopping_opponents_weigth.append(0)



from ortools.linear_solver import pywraplp

def select_best_defensive_midfielder(Market_value,apperence,position_list,seasonal_apperence, goals, assists, againts_team_goals, againts_team_assists, accurate_long_balls_per_game, Big_chances_created_per_game, Block_shots_per_game, Passes_per_game, tackles_accuracy, Crosses_accuracy, Clearence_per_game, Interception_per_game, Fouls_per_game, Duels_won_per_game, Aerial_won_per_game,seasonal_goals,seasonal_assists,seasonal_Yellow,seasonal_Red,seasonal_Second_Yellow,recent_Goals,recent_Assists,recent_Shots,recent_ShotOnTarget,recent_Fouls,recent_Yellow_card,recent_Red_card,recent_Offside,recent_Clearances,recent_Pass_accuracy,recent_Key_passes, weights):
    # Create a solver
    ...
    # Define variables
    num_defensive_midfielder = len(goals)
    x = [solver.IntVar(0, 1, f'x{i}') for i in range(num_defensive_midfielder)]

    # Define objective function
    objective = solver.Objective()
    for i in range(num_defensive_midfielder):
    ...
    # Add constraint
    solver.Add(sum(x[i] for i in range(num_defensive_midfielder)) == 2)  # Select 2 forwards
    
    # Solve the problem
    status = solver.Solve()
    
    if status == pywraplp.Solver.OPTIMAL:
        selected_defensive_midfielder = [i for i in range(num_defensive_midfielder) if x[i].solution_value() == 1]
        return selected_defensive_midfielder
    else:
        return None, None
...
position_list = [1 if position == "Defensive Midfield" else 0 for position in filtered_df["Position"]]
...
weights = {'Market_value':1,'position_list':1,'goals': 4, 'assists': 6,'againts_team_goals' : 4  ,'againts_team_assists' : 6,'accurate_long_balls_per_game': 2+Stealing_the_ball_weigth[0]+Stopping_opponents_weigth[0],'Big_chances_created_per_game': 4+Protecting_the_lead_weigth[0]+Stopping_opponents_weigth[0], 'Block_shots_per_game': 4+additional_weigth[0]+Creating_long_weigth[0]+Keeping_possession_weigth[0],'Passes_per_game' : 2 ,'tackles_accuracy' : 4+additional_weigth[0]+Keeping_possession_weigth[0],'Crosses_accuracy': 2+Stealing_the_ball_weigth[0]+Keeping_possession_weigth[0],'Clearence_per_game' : 4+additional_weigth[0]+Keeping_possession_weigth[0] ,'Interception_per_game' : 4+additional_weigth[0]+Keeping_possession_weigth[0],'Fouls_per_game': 2,'Duels_won_per_game' : 4,'Aerial_won_per_game': 4,'seasonal_goals':6,'seasonal_assists':8,'seasonal_Yellow':2,'seasonal_Red':2,'seasonal_Second_Yellow':2,'recent_Goals':8,'recent_Assists':10,'recent_Shots':4,'recent_ShotOnTarget':4,'recent_Fouls':4,'recent_Yellow_card':4,'recent_Red_card':4,'recent_Offside':2,'recent_Clearances':6,'recent_Pass_accuracy':6,'recent_Key_passes':6}

# Select the best and second-best defensive midfielder
selected_defensive_midfielder = select_best_defensive_midfielder(Market_value,apperence,position_list,seasonal_apperence, goals, assists, againts_team_goals, againts_team_assists, accurate_long_balls_per_game, Big_chances_created_per_game, Block_shots_per_game, Passes_per_game, tackles_accuracy, Crosses_accuracy, Clearence_per_game, Interception_per_game, Fouls_per_game, Duels_won_per_game, Aerial_won_per_game,seasonal_goals,seasonal_assists,seasonal_Yellow,seasonal_Red,seasonal_Second_Yellow,recent_Goals,recent_Assists,recent_Shots,recent_ShotOnTarget,recent_Fouls,recent_Yellow_card,recent_Red_card,recent_Offside,recent_Clearances,recent_Pass_accuracy,recent_Key_passes, weights)

selected_defensive_midfielder.sort(key=lambda x:weights['Market_value'] * Market_value[x]+ weights['position_list'] * position_list[x] + weights['goals'] * (goals[x] / apperence[x]) + weights['assists'] * (assists[x] / apperence[x]) + weights['againts_team_goals'] * againts_team_goals[x] + weights['againts_team_assists'] * againts_team_assists[x] + weights['accurate_long_balls_per_game'] * accurate_long_balls_per_game[x] + weights['Big_chances_created_per_game'] * Big_chances_created_per_game[x] + weights['Block_shots_per_game'] * Block_shots_per_game[x] + weights['Passes_per_game'] * Passes_per_game[x] + weights['tackles_accuracy'] * tackles_accuracy[x] + weights['Crosses_accuracy'] * Crosses_accuracy[x] + weights['Clearence_per_game'] * Clearence_per_game[x] + weights['Interception_per_game'] * Interception_per_game[x] - weights['Fouls_per_game'] * Fouls_per_game[x] + weights['Duels_won_per_game'] * Duels_won_per_game[x] + weights['Aerial_won_per_game'] * Aerial_won_per_game[x] + weights['seasonal_goals'] * (seasonal_goals[x] / seasonal_apperence[x]) + weights['seasonal_assists'] * (seasonal_assists[x] / seasonal_apperence[x]) - weights['seasonal_Yellow'] * seasonal_Yellow[x] - weights['seasonal_Red'] * seasonal_Red[x] - weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[x] + weights['recent_Goals'] * recent_Goals[x] + weights['recent_Assists'] * recent_Assists[x] + weights['recent_Shots'] * recent_Shots[x] + weights['recent_ShotOnTarget'] * recent_ShotOnTarget[x] - weights['recent_Fouls'] * recent_Fouls[x] - weights['recent_Yellow_card'] * recent_Yellow_card[x] - weights['recent_Red_card'] * recent_Red_card[x] - weights['recent_Offside'] * recent_Offside[x]  +  weights['recent_Clearances'] * recent_Clearances[x]+ weights['recent_Pass_accuracy'] * recent_Pass_accuracy[x]+ weights['recent_Key_passes'] * recent_Key_passes[x], reverse=True)



# Get the names of the two best defensive midfielder
best_defensive_midfielder_idx = selected_defensive_midfielder[0]
second_best_defensive_midfielder_idx = selected_defensive_midfielder[1]
best_defensive_midfielder_name = names[best_defensive_midfielder_idx]
second_best_defensive_midfielder_name = names[second_best_defensive_midfielder_idx]
...
if team_strength=='Big six':
    i=best_defensive_midfielder_idx
    per=weights['Market_value'] * Market_value[i]+weights['position_list'] * position_list[i]+ weights['goals'] * (goals[i] / apperence[i]) + weights['assists'] * (assists[i] / apperence[i]) + weights['againts_team_goals'] * againts_team_goals[i] + weights['againts_team_assists'] * againts_team_assists[i] + weights['accurate_long_balls_per_game'] * accurate_long_balls_per_game[i] + weights['Big_chances_created_per_game'] * Big_chances_created_per_game[i] + weights['Block_shots_per_game'] * Block_shots_per_game[i] + weights['Passes_per_game'] * Passes_per_game[i] + weights['tackles_accuracy'] * tackles_accuracy[i] + weights['Crosses_accuracy'] * Crosses_accuracy[i] + weights['Clearence_per_game'] * Clearence_per_game[i] + weights['Interception_per_game'] * Interception_per_game[i] - weights['Fouls_per_game'] * Fouls_per_game[i] + weights['Duels_won_per_game'] * Duels_won_per_game[i] + weights['Aerial_won_per_game'] * Aerial_won_per_game[i] +weights['seasonal_goals'] * (seasonal_goals[i] / seasonal_apperence[i])+weights['seasonal_assists'] * (seasonal_assists[i] / seasonal_apperence[i])-weights['seasonal_Yellow'] * seasonal_Yellow[i]-weights['seasonal_Red'] * seasonal_Red[i]- weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[i]+weights['recent_Goals'] * recent_Goals[i]+weights['recent_Assists'] * recent_Assists[i]+weights['recent_Shots'] * recent_Shots[i]+weights['recent_ShotOnTarget'] * recent_ShotOnTarget[i]-weights['recent_Fouls'] * recent_Fouls[i]-weights['recent_Yellow_card'] * recent_Yellow_card[i]-weights['recent_Red_card'] * recent_Red_card[i]-weights['recent_Offside'] * recent_Offside[i]+  weights['recent_Clearances'] * recent_Clearances[i]+ weights['recent_Pass_accuracy'] * recent_Pass_accuracy[i]+ weights['recent_Key_passes'] * recent_Key_passes[i]
    sheet.append(('Defensive Midfield', best_defensive_midfielder_name,1,per,Jersey_num[i]))
    
elif team_strength=='Good teams':
    if count_2 >6:
        i=second_best_defensive_midfielder_idx
        per=weights['Market_value'] * Market_value[i]+weights['position_list'] * position_list[i]+ weights['goals'] * (goals[i] / apperence[i]) + weights['assists'] * (assists[i] / apperence[i]) + weights['againts_team_goals'] * againts_team_goals[i] + weights['againts_team_assists'] * againts_team_assists[i] + weights['accurate_long_balls_per_game'] * accurate_long_balls_per_game[i] + weights['Big_chances_created_per_game'] * Big_chances_created_per_game[i] + weights['Block_shots_per_game'] * Block_shots_per_game[i] + weights['Passes_per_game'] * Passes_per_game[i] + weights['tackles_accuracy'] * tackles_accuracy[i] + weights['Crosses_accuracy'] * Crosses_accuracy[i] + weights['Clearence_per_game'] * Clearence_per_game[i] + weights['Interception_per_game'] * Interception_per_game[i] - weights['Fouls_per_game'] * Fouls_per_game[i] + weights['Duels_won_per_game'] * Duels_won_per_game[i] + weights['Aerial_won_per_game'] * Aerial_won_per_game[i] +weights['seasonal_goals'] * (seasonal_goals[i] / seasonal_apperence[i])+weights['seasonal_assists'] * (seasonal_assists[i] / seasonal_apperence[i])-weights['seasonal_Yellow'] * seasonal_Yellow[i]-weights['seasonal_Red'] * seasonal_Red[i]- weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[i]+weights['recent_Goals'] * recent_Goals[i]+weights['recent_Assists'] * recent_Assists[i]+weights['recent_Shots'] * recent_Shots[i]+weights['recent_ShotOnTarget'] * recent_ShotOnTarget[i]-weights['recent_Fouls'] * recent_Fouls[i]-weights['recent_Yellow_card'] * recent_Yellow_card[i]-weights['recent_Red_card'] * recent_Red_card[i]-weights['recent_Offside'] * recent_Offside[i]+  weights['recent_Clearances'] * recent_Clearances[i]+ weights['recent_Pass_accuracy'] * recent_Pass_accuracy[i]+ weights['recent_Key_passes'] * recent_Key_passes[i]
        sheet.append(('Defensive Midfield', second_best_defensive_midfielder_name,2,per,Jersey_num[i]))
