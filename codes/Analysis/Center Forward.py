...
#filter players can play center forward
...
#function select center forward
def select_best_center_forward(Market_value,apperence, position_list, seasonal_apperence, goals, assists, againts_team_goals, againts_team_assists, Shots, ShotsOnTarget, Penalty, Free_kick, Big_chances_missed, Shooting_accuracy, Big_chances_created, Crosees, Passes, Off_side, Tackles_per_game, Block_shots_per_game, interceptions_per_game, Total_clearance_per_game, seasonal_goals, seasonal_assists, seasonal_Yellow, seasonal_Red, seasonal_Second_Yellow, recent_Goals, recent_Assists, recent_Shots, recent_ShotOnTarget, recent_Fouls, recent_Yellow_card, recent_Red_card,recent_Pass_accuracy,recent_Keypasses, recent_Offside, weights):
    # Create a solver
    ...
    # Define variables
    num_center_forward = len(goals)
    x = [solver.IntVar(0, 1, f'x{i}') for i in range(num_center_forward)]

    # Define objective function
    objective = solver.Objective()
    for i in range(num_center_forward):
    ...
    # Add constraint
    solver.Add(sum(x[i] for i in range(num_center_forward)) == 2)  # Select 2 forwards
    
    # Solve the problem
    status = solver.Solve()
    
    if status == pywraplp.Solver.OPTIMAL:
        selected_center_forward = [i for i in range(num_center_forward) if x[i].solution_value() == 1]
        return selected_center_forward
    else:
        return None, None
...
position_list = [1 if position == "Centre-Forward" else 0 for position in filtered_df["Position"]]
...
weights = {'Market_value':1,'position_list':1,'goals': 6, 'assists': 4,'againts_team_goals' : 6  ,'againts_team_assists' : 4,'Shots': 4+additional_weigth[0],'ShotsOnTarget': 4+additional_weigth[0], 'Penalty': 2+additional_weigth[0],'Free_kick' : 2+additional_weigth[0] ,'Big_chances_missed' : 4,'Shooting_accuracy': 4,'Big_chances_created' : 2 ,'Crosees' : 1,'Passes': 2,'Off_side' : 2,'seasonal_goals':8,'seasonal_assists':6,'seasonal_Yellow':2,'seasonal_Red':2,'seasonal_Second_Yellow':2,'recent_Goals':10,'recent_Assists':8,'recent_Shots':6,'recent_ShotOnTarget':6,'recent_Fouls':4,'recent_Yellow_card':4,'recent_Red_card':4,'recent_Pass_accuracy':4,'recent_Keypasses':4,'recent_Offside':4}

# Select the best and second-best center forward
selected_center_forward = select_best_center_forward(Market_value,apperence, position_list, seasonal_apperence, goals, assists, againts_team_goals, againts_team_assists, Shots, ShotsOnTarget, Penalty, Free_kick, Big_chances_missed, Shooting_accuracy, Big_chances_created, Crosees, Passes, Off_side, Tackles_per_game, Block_shots_per_game, interceptions_per_game, Total_clearance_per_game, seasonal_goals, seasonal_assists, seasonal_Yellow, seasonal_Red, seasonal_Second_Yellow, recent_Goals, recent_Assists, recent_Shots, recent_ShotOnTarget, recent_Fouls, recent_Yellow_card, recent_Red_card,recent_Pass_accuracy,recent_Keypasses, recent_Offside, weights)

selected_center_forward = list(selected_center_forward)
selected_center_forward.sort(key=lambda x:weights['Market_value'] * Market_value[x]+ weights['position_list'] * position_list[x] + weights['goals'] * (goals[x] / apperence[x]) + weights['assists'] * (assists[x] / apperence[x]) + weights['againts_team_goals'] * againts_team_goals[x] + weights['againts_team_assists'] * againts_team_assists[x] + weights['Shots'] * Shots[x] + weights['ShotsOnTarget'] * ShotsOnTarget[x] + weights['Penalty'] * Penalty[x] + weights['Free_kick'] * Free_kick[x] - weights['Big_chances_missed'] * Big_chances_missed[x] + weights['Shooting_accuracy'] * Shooting_accuracy[x] + weights['Big_chances_created'] * Big_chances_created[x] + weights['Crosees'] * Crosees[x] + weights['Passes'] * Passes[x] - weights['Off_side'] * Off_side[x] + weights['seasonal_goals'] * (seasonal_goals[x] / seasonal_apperence[x]) + weights['seasonal_assists'] * (seasonal_assists[x] / seasonal_apperence[x]) - weights['seasonal_Yellow'] * seasonal_Yellow[x] - weights['seasonal_Red'] * seasonal_Red[x] - weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[x] + weights['recent_Goals'] * recent_Goals[x] + weights['recent_Assists'] * recent_Assists[x] + weights['recent_Shots'] * recent_Shots[x] + weights['recent_ShotOnTarget'] * recent_ShotOnTarget[x] - weights['recent_Fouls'] * recent_Fouls[x] - weights['recent_Yellow_card'] * recent_Yellow_card[x] - weights['recent_Red_card'] * recent_Red_card[x] - weights['recent_Offside'] * recent_Offside[x]+weights['recent_Pass_accuracy']*recent_Pass_accuracy[x]+weights['recent_Keypasses']*recent_Keypasses[x], reverse=True)

# Get the names of the two best center forward
best_center_forward_idx = selected_center_forward[0]
second_best_center_forward_idx = selected_center_forward[1]
best_center_forward_name = names[best_center_forward_idx]
second_best_center_forward_name = names[second_best_center_forward_idx]
...
if team_strength=='Big six':
    i=best_center_forward_idx
    per=weights['Market_value'] * Market_value[i]+ weights['position_list'] * position_list[i] + weights['goals'] * (goals[i] / apperence[i]) + weights['assists'] * (assists[i] / apperence[i]) + weights['againts_team_goals'] * againts_team_goals[i] + weights['againts_team_assists'] * againts_team_assists[i] + weights['Shots'] * Shots[i] + weights['ShotsOnTarget'] * ShotsOnTarget[i] + weights['Penalty'] * Penalty[i] + weights['Free_kick'] * Free_kick[i] - weights['Big_chances_missed'] * Big_chances_missed[i] + weights['Shooting_accuracy'] * Shooting_accuracy[i] + weights['Big_chances_created'] * Big_chances_created[i] + weights['Crosees'] * Crosees[i] + weights['Passes'] * Passes[i] - weights['Off_side'] * Off_side[i] + weights['seasonal_goals'] * (seasonal_goals[i] / seasonal_apperence[i]) + weights['seasonal_assists'] * (seasonal_assists[i] / seasonal_apperence[i]) - weights['seasonal_Yellow'] * seasonal_Yellow[i] - weights['seasonal_Red'] * seasonal_Red[i] - weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[i] + weights['recent_Goals'] * recent_Goals[i] + weights['recent_Assists'] * recent_Assists[i] + weights['recent_Shots'] * recent_Shots[i] + weights['recent_ShotOnTarget'] * recent_ShotOnTarget[i] - weights['recent_Fouls'] * recent_Fouls[i] - weights['recent_Yellow_card'] * recent_Yellow_card[i] - weights['recent_Red_card'] * recent_Red_card[i] - weights['recent_Offside'] * recent_Offside[i]+weights['recent_Pass_accuracy']*recent_Pass_accuracy[i]+weights['recent_Keypasses']*recent_Keypasses[i]
    sheet.append(('Centre-Forward', best_center_forward_name,1,per,Jersey_num[i]))
    
elif team_strength=='Good teams':
    if count_2 >6:
        i=second_best_center_forward_idx
        per=weights['Market_value'] * Market_value[i]+ weights['position_list'] * position_list[i] + weights['goals'] * (goals[i] / apperence[i]) + weights['assists'] * (assists[i] / apperence[i]) + weights['againts_team_goals'] * againts_team_goals[i] + weights['againts_team_assists'] * againts_team_assists[i] + weights['Shots'] * Shots[i] + weights['ShotsOnTarget'] * ShotsOnTarget[i] + weights['Penalty'] * Penalty[i] + weights['Free_kick'] * Free_kick[i] - weights['Big_chances_missed'] * Big_chances_missed[i] + weights['Shooting_accuracy'] * Shooting_accuracy[i] + weights['Big_chances_created'] * Big_chances_created[i] + weights['Crosees'] * Crosees[i] + weights['Passes'] * Passes[i] - weights['Off_side'] * Off_side[i] + weights['seasonal_goals'] * (seasonal_goals[i] / seasonal_apperence[i]) + weights['seasonal_assists'] * (seasonal_assists[i] / seasonal_apperence[i]) - weights['seasonal_Yellow'] * seasonal_Yellow[i] - weights['seasonal_Red'] * seasonal_Red[i] - weights['seasonal_Second_Yellow'] * seasonal_Second_Yellow[i] + weights['recent_Goals'] * recent_Goals[i] + weights['recent_Assists'] * recent_Assists[i] + weights['recent_Shots'] * recent_Shots[i] + weights['recent_ShotOnTarget'] * recent_ShotOnTarget[i] - weights['recent_Fouls'] * recent_Fouls[i] - weights['recent_Yellow_card'] * recent_Yellow_card[i] - weights['recent_Red_card'] * recent_Red_card[i] - weights['recent_Offside'] * recent_Offside[i]+weights['recent_Pass_accuracy']*recent_Pass_accuracy[i]+weights['recent_Keypasses']*recent_Keypasses[i]
        sheet.append(('Centre-Forward', second_best_center_forward_name,2,per,Jersey_num[i]))
